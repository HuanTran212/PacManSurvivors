...
void BossDinoYellow::update(float dt, sf::Vector2f playerPos)
{
...
void BossDinoYellow::draw(sf::RenderWindow& window)
{
...
sf::FloatRect BossDinoYellow::getBounds() const
{
    return m_sprite.getGlobalBounds();
}
void BossDinoYellow::takeDamage(int damage)
{
    m_isHurt = true;
    m_hurtTimer = 3.f;
    m_health -= damage;
...
bool BossDinoYellow::isDead() const
{
    return m_health <= 0.f;
}
...
void BossDinoYellow::setPosition(sf::Vector2f pos)
{
    m_sprite.setPosition(pos);
}

void BossDinoYellow::applySeparation(const std::vector<std::unique_ptr<IEnemy>>& others)
{
